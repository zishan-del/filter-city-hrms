...
function send(res,status,body){
  ...
  res.setHeader('X-FC-Attendance-Rules','Step7C');
...
async function ensureSchema(){
  ...
  await sql`CREATE TABLE IF NOT EXISTS work_schedules (
    employee_id TEXT PRIMARY KEY,work_days TEXT NOT NULL,start_time TIME NOT NULL,end_time TIME NOT NULL,
    break_minutes INTEGER NOT NULL DEFAULT 0,grace_minutes INTEGER NOT NULL DEFAULT 0,
...
module.exports=async(req,res)=>{
  try{
    ...
    if(action==='checkin'){
      if(existing)return send(res,200,{attendance:existing,rule_mode:'existing'});
      ...
      return send(res,201,{attendance:row,rule_mode:rules?'schedule':'legacy'});
    }

    if(!existing)return send(res,400,{error:'Check in first'});

    if(action==='break_start'){
      if(existing.check_out)return send(res,400,{error:'Already checked out'});
      if(existing.break_start&&!existing.break_end)return send(res,400,{error:'Break already started'});
      ...
      return send(res,200,{attendance:row});
    }

    if(action==='break_end'){
      if(!existing.break_start)return send(res,400,{error:'Start break first'});
      if(existing.break_end)return send(res,400,{error:'Break already ended'});
      ...
      return send(res,200,{attendance:row,rule_mode:rules?'schedule':'legacy'});
    }

    if(action==='checkout'){
      if(existing.check_out)return send(res,400,{error:'Already checked out'});
      if(existing.break_start&&!existing.break_end)return send(res,400,{error:'End your break before checking out'});
      ...
      return send(res,200,{attendance:row,rule_mode:mode,rules:{classification:rules.classification,expected_work_minutes:rules.expected,break_over_minutes:rules.breakOver}});
    }

    return send(res,400,{error:'Unknown attendance action'});
  }catch(error){
    console.error('Step 7C attendance error',error);
    return send(res,500,{error:error.message||'Attendance error'});
  }
};
