(function(){
  ...
  function riyadhMonth(){
    const p={};
    new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Riyadh',year:'numeric',month:'2-digit'}).formatToParts(new Date()).forEach(x=>{if(x.type!=='literal')p[x.type]=x.value;});
  ...
  function correctionTime(v){
    if(!v)return '—';
    try{return new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Riyadh',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date(v));}
  ...
  function parseDetails(details){
    ...
    let reason='—',recalculated='',schedule='';
    const changes=[];

    const firstFieldPositions=fieldLabels
      .map(label=>text.indexOf('; '+label+': '))
      .filter(index=>index>=0)
      .sort((a,b)=>a-b);
    if(text.startsWith('Reason: ')){
      let end=firstFieldPositions.length?firstFieldPositions[0]:text.indexOf('; Recalculated:');
      if(end<0)end=text.length;
      reason=text.slice(8,end).trim()||'—';
    }

    parts.forEach(part=>{
      const label=fieldLabels.find(name=>part.startsWith(name+': '));
      if(label){
        const value=part.slice(label.length+2);
        const arrow=value.indexOf(' -> ');
        if(arrow>=0)changes.push({field:label,oldValue:value.slice(0,arrow),newValue:value.slice(arrow+4)});
      }else if(part.startsWith('Recalculated: ')){
        recalculated=part.slice('Recalculated: '.length);
      }else if(part.startsWith('Schedule ')){
        schedule=part.slice('Schedule '.length);
      }
    });

    if(!changes.length)changes.push({field:'Correction',oldValue:'—',newValue:'—'});
    return {reason,changes,recalculated,schedule};
  }
  ...
  function build(employeeId,month){
    ...
    events.forEach(a=>{
      ...
      parsed.changes.forEach(c=>rows.push({
        ...
        employee:employeeName(a.employee_id),
        workDate:dateOnly(a.work_date),
        correctionTime:correctionTime(a.created_at),
        field:c.field,
        oldValue:c.oldValue,
        newValue:c.newValue,
        reason:parsed.reason,
        admin:a.actor_username||a.actor_role||'Admin',
        recalculated:parsed.recalculated,
    ...
    return {
      events,
      rows,
      employees:new Set(events.map(x=>x.employee_id)),
      attendance:new Set(events.map(x=>String(x.attendance_id))),
      sourceCount:auditRows.length
    };
  }
  ...
  function reportHtml(data,employeeId,month){
    ...
    const body=data.rows.length?data.rows.map(r=>`<tr>
      <td>${esc9b(r.correctionTime)}</td>
      <td>${esc9b(r.employeeId)} — ${esc9b(r.employee)}</td>
      <td>${esc9b(r.workDate)}</td>
      <td><span class="badge">${esc9b(r.field)}</span></td>
      <td style="white-space:normal;min-width:170px">${esc9b(r.oldValue)}</td>
      <td style="white-space:normal;min-width:170px">${esc9b(r.newValue)}</td>
      <td style="white-space:normal;min-width:220px">${esc9b(r.reason)}</td>
      <td>${esc9b(r.admin)}</td>
      <td>${esc9b(r.auditId)}</td>
    </tr>`).join(''):`<tr><td colspan="9" class="empty">No ADMIN_CORRECTION events for ${esc9b(scope)} in ${esc9b(monthLabel(month))}.</td></tr>`;
    return `<div class="section" style="margin-top:14px">
      <div class="section-head"><div><b>${esc9b(scope)} · ${esc9b(monthLabel(month))}</b><div class="muted" style="font-size:12px;margin-top:4px">Permanent attendance correction audit trail · Saudi/Riyadh time</div></div><span class="badge">Read only · no DB writes</span></div>
      <div class="cards" style="padding:16px">
        <div class="card">Correction Events<div class="stat">${data.events.length}</div></div>
        <div class="card">Employees Corrected<div class="stat">${data.employees.size}</div></div>
        <div class="card">Attendance Records<div class="stat">${data.attendance.size}</div></div>
        <div class="card">Fields Changed<div class="stat">${data.rows.length}</div></div>
      </div>
      ${warning}
      <div class="table-wrap"><table class="table"><thead><tr><th>Correction Time</th><th>Employee</th><th>Attendance Date</th><th>Field Changed</th><th>Old Value</th><th>New Value</th><th>Reason</th><th>Admin</th><th>Audit ID</th></tr></thead><tbody>${body}</tbody></table></div>
    </div>`;
  }
  ...
  window.fc9bRender=async function(){
    ...
    try{
      ...
      const employeeId=selectedEmployee(),month=selectedMonth(),data=build(employeeId,month);
      reportCache[employeeId+'|'+month]=data;
      holder.innerHTML=reportHtml(data,employeeId,month);
    }catch(e){
      holder.innerHTML='<div class="empty">'+esc9b(e.message||'Could not load correction report')+'</div>';
      if(typeof toast==='function')toast(e.message||'Could not load correction report');
    }
  };
  ...
  window.fc9bExportCsv=async function(){
    try{
      await loadAudit(false);
      const employeeId=selectedEmployee(),month=selectedMonth(),data=reportCache[employeeId+'|'+month]||build(employeeId,month);
      const out=[['Audit ID','Attendance ID','Correction Time Riyadh','Employee ID','Employee','Attendance Date','Field Changed','Old Value','New Value','Reason','Admin','Recalculated','Schedule']];
      data.rows.forEach(r=>out.push([r.auditId,r.attendanceId,r.correctionTime,r.employeeId,r.employee,r.workDate,r.field,r.oldValue,r.newValue,r.reason,r.admin,r.recalculated,r.schedule]));
      const csv=out.map(row=>row.map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(',')).join('\n');
      const a=document.createElement('a');
      a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));
      a.download=`attendance-corrections-${employeeId||'all'}-${month}.csv`;
      a.click();
      setTimeout(()=>URL.revokeObjectURL(a.href),1000);
    }catch(e){if(typeof toast==='function')toast(e.message||'Could not export correction report');}
  };
  ...
  window.fc9bPrint=async function(){
    try{
      ...
      if(!w)return toast('Allow pop-ups to print the correction report');
